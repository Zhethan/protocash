...
impl KVStore {
    fn info(&self) -> response::Info {
        ...
        tracing::info!("info -> height: {}, app_hash: {:?}", self.height, self.app_hash);
        response::Info {
    ...
    fn query(&self, query: Bytes) -> response::Query {
        ...
        tracing::info!("query -> key: {}, value: {}", key, value);
        response::Query {
            log,
    ...
    fn execute_tx(&mut self, tx: Bytes) -> ExecTxResult {
        ...
        tracing::info!("execute_tx -> inserted key-value pair: {}={}", key, value);
        ExecTxResult {
